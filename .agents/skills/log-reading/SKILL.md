...
4. **Log the session**:
   ...
   - If the book already existed:
     ```bash
     python3 scripts/log_reading.py log -k "<Kid>" -b "<Book Title>" -m <Minutes> [--percentage <percent> | -p <page>]
     ```
   - If the book is new (create on the fly using retrieved web metadata):
     ```bash
     python3 scripts/log_reading.py log -k "<Kid>" -b "<Book Title>" -m <Minutes> --create-if-missing --author "<Author>" --total-pages <Total Pages> [--percentage <percent> | -p <page>]
     ```
     *(Add `--cover "<Cover URL>"` if a cover was found).*
   
5. **Confirm with the user**:
   - Present a concise summary of the action taken (e.g., book title, author, minutes logged, updated page progress).
   - When displaying reading lists, logs, or stats to the user, **DO NOT include rating information** (e.g., average rating or stars), as the user does not care about it.
